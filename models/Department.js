...
class Department {
  ...
  static async create(name) {
    const sql = 'INSERT INTO assets_departments (name) VALUES (@name)';
    return await execute(sql, { name });
  }

  // 取得所有部門
  static async findAll() {
    const sql = 'SELECT * FROM assets_departments ORDER BY name';
    return await query(sql);
  }

  // 根據 ID 尋找部門
  static async findById(id) {
    const sql = 'SELECT * FROM assets_departments WHERE id = @id';
  ...
  static async update(id, name) {
    const sql = 'UPDATE assets_departments SET name = @name WHERE id = @id';
    return await execute(sql, { id, name });
  }

  // 刪除部門
  static async delete(id) {
    const sql = 'DELETE FROM assets_departments WHERE id = @id';
    return await execute(sql, { id });
  }

  // 檢查部門名稱是否已存在
  static async nameExists(name, excludeId = null) {
    let sql = 'SELECT COUNT(*) as count FROM assets_departments WHERE name = @name';
    const params = { name };
  ...
  static async search(searchTerm) {
    const sql = 'SELECT * FROM assets_departments WHERE name LIKE @search ORDER BY name';
    return await query(sql, { search: `%${searchTerm}%` });
  }

  // 取得部門數量
  static async count() {
    const sql = 'SELECT COUNT(*) as total FROM assets_departments';
  ...
  static async getRecent(limit = 5) {
    const sql = `SELECT TOP ${limit} * FROM assets_departments ORDER BY created_at DESC`;
    return await query(sql);
  }
}
